...
describe('CelebrationThrottle', () => {
  it('blocks small wins inside 45s window', () => {
    ...
    expect(t.allow({ kind: 'save', size: 'small' }, 1000)).toBe(true);
    expect(t.allow({ kind: 'save', size: 'small' }, 20_000)).toBe(false);
    expect(t.allow({ kind: 'save', size: 'small' }, 46_000)).toBe(true);
  });

  it('always allows big wins', () => {
    const t = new CelebrationThrottle(45_000);
    t.allow({ kind: 'save', size: 'small' }, 1000);
  ...
  it('always allows commit wins even inside small-win window', () => {
    const t = new CelebrationThrottle(45_000);
    expect(t.allow({ kind: 'save', size: 'small' }, 1000)).toBe(true);
    expect(t.allow({ kind: 'commit', size: 'small' }, 2000)).toBe(true);
    expect(t.allow({ kind: 'commit', size: 'small' }, 3000)).toBe(true);
    expect(t.allow({ kind: 'save', size: 'small' }, 4000)).toBe(false);
  });
